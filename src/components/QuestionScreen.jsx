...
function QuestionScreen({ state, dispatch }) {
  ...
  return (
    ...
    <div className="text-white flex w-[100%] justify-between items-center p-3"><span className="bg-white px-7 py-2 rounded-full text-sky-900 font-bold">Name: {name.toUpperCase()}</span><span className="bg-white px-7 py-2 text-sky-900 font-bold rounded-full">Level: {level.toUpperCase()}</span></div>
      <div className="h-[80%] w-[75%] rounded-md p-5 space-y-16">
        <progress value={index + Number(answer !== null)} max={quesAccordingLevel.length} className=" block w-full " />
        ...
        <div className="flex justify-between">
         <Timer state={state} dispatch={dispatch} />
          {index === quesAccordingLevel.length - 1 ? <Button  onClick={() => dispatch({ type: "finishQuiz"})} btnType="primary">Finish</Button> :<Button onClick={() => dispatch({ type: "nextQuestion", payload: answer === correctOption ? question.points : 0 })} btnType="primary">Next</Button> }
